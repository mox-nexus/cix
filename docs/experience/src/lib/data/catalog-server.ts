...
export function loadAllExtensions(): CatalogExtension[] {
	const plugins = loadPlugins();
	const tools = loadTools();
	return [...plugins, ...tools].sort((a, b) => a.slug.localeCompare(b.slug));
}

export function findExtension(slug: string): CatalogExtension | undefined {
	return loadAllExtensions().find((ext) => ext.slug === slug);
}
