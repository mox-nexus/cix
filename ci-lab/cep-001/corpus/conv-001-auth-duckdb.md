...
yza: We need to decide where to store auth tokens. Redis vs DuckDB vs SQLite.
...
yza: We're single-node for now. Analytics on token usage patterns matters — we want to query "which services consume the most tokens per hour?"
