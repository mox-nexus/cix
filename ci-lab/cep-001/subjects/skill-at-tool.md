---
name: skill-at-tool
description: memex described via --skill output (semantic layer, no --help)
runtime:
  type: claude
  max_turns: 10
  allowed_tools:
...
| DuckDB lock error during backfill | Normal — wait for backfill to finish |

## Key Facts

- **Embedding model**: nomic-embed-text-v1.5 (768-dim), via fastembed (ONNX)
...
- **Fusion**: RRF (k=60)
- **Backfill is idempotent**: resumes from where it left off
